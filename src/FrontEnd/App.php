...
final class App
{
    public static function routes(): array
    {
        ...
        if(class_exists(Views\Insurance::class)){$routes[]=['name'=>'insurance','pattern'=>'insurance','label'=>'Insurance','nav'=>true,'view'=>'insurance'];$routes[]=['name'=>'insurance-detail','pattern'=>'insurance/medicare-dme-oxygen','label'=>'Payer requirement','nav'=>false,'view'=>'insurance-detail'];}
        if(class_exists(Views\Playbooks::class)){$routes[]=['name'=>'playbooks','pattern'=>'playbooks','label'=>'Playbooks','nav'=>true,'view'=>'playbooks'];$routes[]=['name'=>'playbook-detail','pattern'=>'playbooks/arrange-home-oxygen','label'=>'Playbook','nav'=>false,'view'=>'playbook-detail'];}
        if(class_exists(Views\KnowledgeBase::class)){$routes[]=['name'=>'knowledgebase','pattern'=>'knowledgebase','label'=>'Knowledge Base','nav'=>true,'view'=>'knowledgebase'];$routes[]=['name'=>'knowledge-detail','pattern'=>'knowledgebase/discharge-documentation-standard','label'=>'Knowledge article','nav'=>false,'view'=>'knowledge-detail'];}
        return apply_filters('atlas_frontend_routes',$routes);
    }
    ...
    public function render(string $route): void
    {
        ...
        wp_enqueue_style('atlas-expansion',ATLAS_URL.'assets/expansion.css',['atlas-app'],ATLAS_VERSION);
        wp_enqueue_script('atlas-app',ATLAS_URL.'assets/app.js',[],ATLAS_VERSION,true);
        $view=$routes[$route]['view'];
        $content=match($view){
            'home'=>$this->home(),'resources'=>$this->resources(),'resource-detail'=>$this->resourceDetail(),
            'insurance'=>Views\Insurance::library(),'insurance-detail'=>Views\Insurance::detail(),
            'playbooks'=>Views\Playbooks::library(),'playbook-detail'=>Views\Playbooks::detail(),
            'knowledgebase'=>Views\KnowledgeBase::library(),'knowledge-detail'=>Views\KnowledgeBase::detail(),
            default=>apply_filters('atlas_render_view','',$view)
        };
    ...
    private function resources(): string
    {
        ...
        <div class="atlas-results-head"><strong><?php echo count($items);?> results</strong><span class="atlas-muted">Visible trust and review context</span></div><div class="atlas-card-grid"><?php foreach($items as$r):$url=$r['slug']==='home-oxygen-qualification'?home_url('/resources/home-oxygen-qualification'):'#';?><article class="atlas-resource-card"><div class="atlas-card-meta"><span class="atlas-badge"><?php echo esc_html($r['category']);?></span><span class="atlas-trust">● <?php echo esc_html($r['trust']);?></span></div><h2><a href="<?php echo esc_url($url);?>"><?php echo esc_html($r['title']);?></a></h2><p><?php echo esc_html($r['summary']);?></p><dl><div><dt>Source</dt><dd><?php echo esc_html($r['source']);?></dd></div><div><dt>Reviewed</dt><dd><?php echo esc_html($r['reviewed']);?></dd></div></dl><div class="atlas-card-footer"><span><?php echo esc_html($r['audience']);?></span><a href="<?php echo esc_url($url);?>">Open →</a></div></article><?php endforeach;?></div><?php if(!$items):?><div class="atlas-empty"><h2>No resources match that search</h2><p>Try a broader term or clear the category filter.</p></div><?php endif;return(string)ob_get_clean();
    }

    private function resourceDetail(): string
    {
        $r=Fixtures::resource('home-oxygen-qualification');if(!$r)return'<div class="atlas-empty"><h1>Demo content disabled</h1></div>';ob_start();?><nav class="atlas-breadcrumb"><a href="<?php echo esc_url(home_url('/resources'));?>">Resources</a><span>/</span><span>Home Oxygen</span></nav><article class="atlas-detail"><header class="atlas-detail-head"><div><div class="atlas-card-meta"><span class="atlas-badge"><?php echo esc_html($r['category']);?></span><span class="atlas-trust">● <?php echo esc_html($r['trust']);?></span></div><h1><?php echo esc_html($r['title']);?></h1><p><?php echo esc_html($r['summary']);?></p></div><aside class="atlas-trust-panel"><span>Trust snapshot</span><strong><?php echo esc_html($r['source']);?></strong><small>Reviewed <?php echo esc_html($r['reviewed']);?></small><button class="atlas-button atlas-button-secondary" onclick="window.print()">Print resource</button></aside></header><div class="atlas-detail-layout"><div class="atlas-prose"><section><p class="atlas-eyebrow">At a glance</p><h2>Key points</h2><ul class="atlas-check-list"><?php foreach($r['key_points']as$p):?><li><?php echo esc_html($p);?></li><?php endforeach;?></ul></section><section><p class="atlas-eyebrow">Operational sequence</p><h2>From qualification to delivery</h2><ol class="atlas-step-list"><?php foreach($r['workflow']as$i=>$s):?><li><span><?php echo $i+1;?></span><strong><?php echo esc_html($s);?></strong></li><?php endforeach;?></ol></section><section class="atlas-source-box"><p class="atlas-eyebrow">Source citation</p><p><?php echo esc_html($r['citation']);?></p></section></div><aside class="atlas-related"><p class="atlas-eyebrow">Connected in Atlas</p><h2>Related guidance</h2><a href="<?php echo esc_url(home_url('/playbooks/arrange-home-oxygen'));?>"><strong>Arrange Home Oxygen</strong><span>Open Playbook →</span></a><a href="<?php echo esc_url(home_url('/insurance/medicare-dme-oxygen'));?>"><strong>Medicare DME: Oxygen</strong><span>Open payer rule →</span></a><a href="<?php echo esc_url(home_url('/patient-resources/home-oxygen-what-to-expect'));?>"><strong>Home Oxygen: What to Expect</strong><span>Open patient handout →</span></a></aside></div></article><?php return(string)ob_get_clean();
    }

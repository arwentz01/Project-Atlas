...
final class App
{
    ...
    {
        $routes=[
            ['name'=>'home','pattern'=>'','label'=>'Home','nav'=>true,'view'=>'home'],
    ...
    {
        $u=wp_get_current_user();?><!doctype html><html <?php language_attributes();?>><head><meta charset="<?php bloginfo('charset');?>"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title><?php echo esc_html($title.' | Atlas');?></title><?php wp_head();?></head><body class="atlas-body"><a class="atlas-skip-link" href="#atlas-main">Skip to main content</a><div class="atlas-app"><aside class="atlas-sidebar" id="atlas-sidebar"><a class="atlas-brand" href="<?php echo esc_url(home_url('/'));?>"><span class="atlas-brand-mark">A</span><span>Atlas</span></a><nav aria-label="Atlas navigation"><?php foreach(self::routes()as$r):if(empty($r['nav']))continue;$active=$current===$r['name']||str_starts_with($current,$r['name'].'-');$target=trim((string)$r['pattern'],'/');?><a href="<?php echo esc_url($target===''?home_url('/'):home_url('/'.$target));?>" <?php echo $active?'aria-current="page"':'';?>><span class="atlas-nav-dot"></span><?php echo esc_html($r['label']);?></a><?php endforeach;?></nav><div class="atlas-sidebar-footer"><span class="atlas-avatar"><?php echo esc_html(strtoupper(substr($u->display_name?:$u->user_login,0,1)));?></span><div><strong><?php echo esc_html($u->display_name?:$u->user_login);?></strong><span>Atlas Demo Health</span></div></div></aside><div class="atlas-workspace"><header class="atlas-topbar"><button class="atlas-menu-button" data-atlas-menu aria-controls="atlas-sidebar" aria-expanded="false">Menu</button><span class="atlas-context">Atlas Demo Health</span><div class="atlas-top-actions"><a href="<?php echo esc_url(home_url('/search'));?>">Search</a><a href="<?php echo esc_url(wp_logout_url(home_url('/')));?>">Sign out</a></div></header><main class="atlas-main" id="atlas-main"><?php echo $content;// phpcs:ignore?></main></div></div><?php wp_footer();?></body></html><?php
    }
}

...
final class SchedulingRepository
{
    ...
    public function createShift(int $organizationId,int $userId,array $input): int
    {
        ...
        if(empty($input['shift_date'])||empty($input['starts_at'])||empty($input['ends_at']))throw new InvalidArgumentException('Shift date, start time, and end time are required.');$startStamp=strtotime($input['shift_date'].' '.$input['starts_at']);$endStamp=strtotime($input['shift_date'].' '.$input['ends_at']);if($endStamp<=$startStamp)$endStamp=strtotime('+1 day',$endStamp);$duration=($endStamp-$startStamp)/3600;if($duration<=0||$duration>16)throw new InvalidArgumentException('Shifts may cross midnight but must be 16 hours or shorter.');
    ...
    {
        if(!in_array($decision,['approved','rejected'],true))throw new InvalidArgumentException('Invalid time-entry decision.');$q=$this->db->prepare('SELECT COUNT(*) FROM time_entries te JOIN labor_periods lp ON lp.organization_id=te.organization_id AND DATE(te.clocked_in_at) BETWEEN lp.starts_on AND lp.ends_on AND lp.status="locked" WHERE te.id=? AND te.organization_id=?');$q->execute([$entryId,$oid]);if($q->fetchColumn())throw new InvalidArgumentException('This labor period is locked.');$q=$this->db->prepare('UPDATE time_entries SET status=?,manager_note=?,reviewed_by=?,reviewed_at=NOW() WHERE id=? AND organization_id=? AND status="submitted"');$q->execute([$decision,trim($note)?:null,$uid,$entryId,$oid]);if(!$q->rowCount())throw new InvalidArgumentException('Submitted time entry unavailable.');$this->audit($oid,$uid,'time_entry.'.$decision,(string)$entryId);
    }
    ...
    {
        $q=$this->db->prepare('SELECT week_starts_on FROM master_schedule_generations WHERE id=? AND organization_id=?');$q->execute([$generationId,$oid]);$week=$q->fetchColumn();if(!$week)throw new InvalidArgumentException('Draft generation unavailable.');$this->db->beginTransaction();try{$this->db->prepare('INSERT INTO master_generation_publications (generation_id,status,published_by,published_at) VALUES (?,"published",?,NOW()) ON DUPLICATE KEY UPDATE status="published",published_by=VALUES(published_by),published_at=VALUES(published_at)')->execute([$generationId,$uid]);$q=$this->db->prepare('SELECT DISTINCT s.assigned_membership_id FROM master_schedule_generation_items mgi JOIN shifts s ON s.id=mgi.shift_id WHERE mgi.generation_id=? AND s.assigned_membership_id IS NOT NULL');$q->execute([$generationId]);$notify=$this->db->prepare('INSERT INTO notifications (organization_id,membership_id,notification_type,title,body,action_route) VALUES (?,? ,"schedule_published","Schedule published",?,"my-schedule")');foreach($q->fetchAll(PDO::FETCH_COLUMN) as $member){$notify->execute([$oid,$member,'Your schedule for the week of '.$week.' is available.']);$notification=(int)$this->db->lastInsertId();$this->db->prepare('INSERT INTO notification_deliveries (organization_id,membership_id,notification_id,channel,status) VALUES (?,?,?,"in_app","sent")')->execute([$oid,$member,$notification]);$pref=$this->db->prepare('SELECT email_enabled FROM notification_preferences WHERE membership_id=? AND organization_id=?');$pref->execute([$member,$oid]);if($pref->fetchColumn())$this->db->prepare('INSERT INTO notification_deliveries (organization_id,membership_id,notification_id,channel,status,next_attempt_at) VALUES (?,?,?,"email","queued",NOW())')->execute([$oid,$member,$notification]);}$this->audit($oid,$uid,'master_schedule.published',(string)$generationId);$this->db->commit();}catch(Throwable $e){if($this->db->inTransaction())$this->db->rollBack();throw $e;}
    }
    ...
    {
        $shift=$this->owned('shifts',$oid,$in['shift_id']??null,true);$q=$this->db->prepare('SELECT * FROM shifts WHERE id=? AND organization_id=?');$q->execute([$shift,$oid]);$before=$q->fetch();$date=(string)($in['shift_date']??$before['shift_date']);$start=(string)($in['starts_at']??$before['starts_at']);$end=(string)($in['ends_at']??$before['ends_at']);$member=!empty($in['membership_id'])?$this->membership($oid,$in['membership_id']):null;$startStamp=strtotime($date.' '.$start);$endStamp=strtotime($date.' '.$end);if($endStamp<=$startStamp)$endStamp=strtotime('+1 day',$endStamp);$duration=($endStamp-$startStamp)/3600;if(!$date||!$start||!$end||$duration<=0||$duration>16)throw new InvalidArgumentException('Shifts may cross midnight but must be 16 hours or shorter.');$this->db->beginTransaction();try{$this->db->prepare('UPDATE shifts SET shift_date=?,starts_at=?,ends_at=?,assigned_membership_id=?,status=? WHERE id=? AND organization_id=?')->execute([$date,$start,$end,$member,$member?'assigned':'open',$shift,$oid]);if($member){$elig=$this->eligibility($oid,$member,$shift,$shift);if($elig['result']!=='eligible'&&!empty($in['require_eligible']))throw new InvalidArgumentException(implode(' ',$elig['reasons']));}$q=$this->db->prepare('SELECT * FROM shifts WHERE id=?');$q->execute([$shift]);$after=$q->fetch();$this->db->prepare('INSERT INTO shift_history (organization_id,shift_id,action,before_json,after_json,reason,changed_by) VALUES (?,? ,"moved",?,?,?,?)')->execute([$oid,$shift,json_encode($before),json_encode($after),trim((string)($in['reason']??''))?:null,$uid]);$this->db->commit();$this->audit($oid,$uid,'shift.moved',(string)$shift);}catch(Throwable $e){if($this->db->inTransaction())$this->db->rollBack();throw $e;}
    }

...
final class PacketController
{
    ...
    public function options(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        try {
            ...
            return new WP_REST_Response(['resources' => $this->packets->patientResourceOptions($org?->id, max(1, (int) ($request->get_param('per_page') ?: 50)))], 200);
        } catch (Throwable $e) { return $this->error($e, 'atlas_packet_options_unavailable'); }
    }
    ...
    public function snapshot(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        try {
            $org = $this->orgs->resolveForUser(get_current_user_id());
            $id = $this->packets->createSnapshot((string) $request->get_param('id'), get_current_user_id(), $org?->id);
            return $id === null ? new WP_Error('atlas_packet_not_found', __('The packet was not found or is not accessible.', 'atlas-platform'), ['status' => 404]) : new WP_REST_Response(['id' => $id], 201);
        } catch (Throwable $e) { return $this->error($e, 'atlas_packet_snapshot_failed'); }
    }

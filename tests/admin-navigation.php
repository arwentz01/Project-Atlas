...
$assert(count($items) === 2 && $items[0]['slug'] === 'atlas' && $items[0]['current'] && $items[1]['slug'] === 'atlas-personal', 'Atlas users see implemented authorized destinations');
$GLOBALS['atlas_test_capabilities'][] = 'atlas_view_diagnostics';
$items = $navigation->visible('atlas-diagnostics');
$assert(count($items) === 3 && $items[1]['current'], 'diagnostic navigation visibility and current state share capability policy');
